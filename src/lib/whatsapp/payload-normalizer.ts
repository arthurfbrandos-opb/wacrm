...
function textBody(m: UazAPIBaseMessage): string | undefined {
  ...
  return m.text.message || m.text.body || undefined
}
...
function normalizeMessage(m: UazAPIBaseMessage) {
  ...
  const ts = String(m.messageTimestamp || m.timestamp || Math.floor(Date.now() / 1000))
