...
export function MessageComposer({
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `Arquivo com ${(file.size / 1024 / 1024).toFixed(1)} MB — limite para ${kind} é ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          {/* Attach menu — photo / video / document / voice.
              Hidden for UazAPI: the send route rejects media on that channel. */}
          {!isUazapi && (
            <DropdownMenu>
              <DropdownMenuTrigger
          ...
          {/* Template button — hidden for UazAPI (no template support on that channel). */}
          {!isUazapi && (
            <GatedButton
              variant="ghost"
              size="sm"
